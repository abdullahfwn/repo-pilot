...
export default function Dashboard({ data, repoUrl, onBack }: DashboardProps) {
  ...
  return (
    <motion.div 
      ...
      className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12 text-abyssal font-mono"
    >
      
      {/* 1. Header Section */}
      <header className="mb-10 flex flex-col items-start gap-4">
        <div className="flex items-center gap-6 w-full">
          <button onClick={onBack} className="p-3 hover:bg-white hover:shadow-sm border border-transparent hover:border-oatmeal rounded-full transition-all text-abyssal/60 hover:text-abyssal group">
            <ArrowLeft className="h-6 w-6 group-hover:-translate-x-1 transition-transform" />
          </button>
          <img src={data.repoDetails?.ownerAvatar || ''} alt="Owner" className="w-20 h-20 rounded-2xl border-2 border-oatmeal shadow-xl" />
          <div>
            <h1 className="text-5xl font-serif font-normal text-abyssal flex items-center gap-3">
              {data.repoDetails?.name || 'Unknown Repository'}
              <a href={data.repoDetails?.url} target="_blank" rel="noreferrer" className="text-abyssal/40 hover:text-truffle transition-colors">
                <Github className="h-6 w-6" />
              </a>
            </h1>
            <p className="text-abyssal/70 max-w-2xl text-lg mt-2 leading-relaxed font-mono">{data.repoDetails?.description || 'No description available'}</p>
          </div>
        </div>
      </header>
      ...
      <section className="mb-24">
        ...
        <div className="bg-white/40 border border-oatmeal rounded-3xl p-8 space-y-8 shadow-sm">
      ...
      <section className="mb-24">
         ...
         <div className="bg-white/40 border border-oatmeal rounded-3xl p-8 shadow-sm">
      ...
      <section className="mb-24">
         ...
         <div className="bg-white/40 border border-oatmeal rounded-3xl p-8 shadow-sm">
            <div className="bg-abyssal rounded-xl p-6 border border-abyssal font-mono text-sm space-y-3 overflow-x-auto shadow-inner text-palladian">
                {howToRun.map((step, i) => (
      ...
      <section className="mb-24">
         ...
         <div className="bg-white/40 border border-oatmeal rounded-3xl p-8 shadow-sm">
            <div className="flex flex-col lg:flex-row items-center justify-between gap-12">
                ...
                <div className="w-full lg:w-1/2 h-[450px] relative font-mono">
                    <div className="absolute inset-0 bg-burningFlame/10 rounded-full blur-3xl" />
      ...
      <section className="mb-24">
         ...
         <div className="grid md:grid-cols-2 gap-6">
            <div className="bg-white/30 border border-oatmeal/60 rounded-3xl p-8">
            ...
            <div className="bg-truffle/5 border border-truffle/10 rounded-3xl p-8">
      ...
      <section className="mb-24">
         ...
         <div className="grid lg:grid-cols-2 gap-10">
            ...
            <div className="bg-white/40 border border-oatmeal rounded-3xl p-8 flex flex-col justify-center relative overflow-hidden shadow-sm">
                ...
                <div className="space-y-3">
                    {architectureLayers.length > 0 ? (
                        architectureLayers.map((layer, idx) => (
                            <div key={idx} className="bg-white border border-oatmeal p-4 rounded-xl text-center shadow-md relative z-10 mx-4 transform hover:scale-105 transition-transform duration-300">
                                <span className="font-mono text-truffle font-bold tracking-wider">{layer}</span>
                            </div>
                        ))
                    ) : (
                         // Fallback visualization if no specific layers returned
                         <>
                            <div className="bg-white border border-oatmeal p-4 rounded-xl text-center shadow-md mx-4 opacity-100"><span className="font-mono text-truffle">Presentation Layer</span></div>
                            <div className="h-4 border-l-2 border-dashed border-oatmeal mx-auto"></div>
                            <div className="bg-white border border-oatmeal p-4 rounded-xl text-center shadow-md mx-4 opacity-90"><span className="font-mono text-truffle">Business Logic</span></div>
                            <div className="h-4 border-l-2 border-dashed border-oatmeal mx-auto"></div>
                            <div className="bg-white border border-oatmeal p-4 rounded-xl text-center shadow-md mx-4 opacity-80"><span className="font-mono text-truffle">Data Access</span></div>
                         </>
                    )}
                </div>
            </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6 mb-24 items-start">
        
        {/* Project Structure */}
        <div className="bg-white/40 border border-oatmeal rounded-3xl p-6 flex flex-col min-h-[400px] max-h-[800px] shadow-sm">
      ...
      <div className="flex justify-center mb-12">
        <button
          onClick={onBack}
          className="bg-abyssal hover:bg-truffle text-palladian px-8 py-4 rounded-xl font-bold transition-all shadow-lg shadow-abyssal/20 hover:shadow-truffle/20 flex items-center gap-3 text-lg font-mono border border-transparent hover:border-oatmeal"
        >
...
function OverallScoreCircle({ score }: { score: number }) {
  ...
  return (
    <div className="bg-palladian rounded-3xl p-6 border border-oatmeal shadow-md relative">
      <div className="flex justify-between items-center mb-6 relative z-10">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-white rounded-lg border border-oatmeal shadow-sm">
            <BarChart3 className="w-5 h-5 text-abyssal" />
          </div>
          <span className="font-serif text-xl text-abyssal">Repository Score</span>
        </div>
        <span className={`px-3 py-1 rounded-full text-xs font-mono border ${ratingStyles} font-bold uppercase tracking-wide`}>
          {rating}
        </span>
      </div>
...
function SectionHeader({ icon, title }: { icon: React.ReactNode, title: string }) {
    return (
        <div className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-4xl font-serif font-normal text-abyssal">{title}</h2>
        </div>
    )
}
...
function ScoreCard({ label, score }: { label: string, score: number }) {
  ...
  return (
    <div className="bg-white p-5 rounded-xl border border-oatmeal flex justify-between items-center hover:border-abyssal transition-colors shadow-sm">
        <span className="text-abyssal/70 font-medium text-lg font-mono">{label}</span>
        <div className="flex items-center gap-4">
             <div className="w-32 h-3 bg-palladian border border-oatmeal/30 rounded-full overflow-hidden">
                <div className={`h-full ${color}`} style={{ width: `${score}%` }}></div>
             </div>
             <span className={`font-mono font-bold w-10 text-right text-xl ${textColor}`}>{score}</span>
        </div>
    </div>
  );
}
...
function StatBadge({ icon, label, value }: { icon: React.ReactNode, label: string, value: number }) {
  return (
    <div className="flex flex-col items-center bg-white border border-oatmeal px-8 py-4 rounded-2xl min-w-[120px] shadow-sm">
      <div className="flex items-center gap-2 mb-2">
        {icon}
        <span className="text-xs text-abyssal/50 font-bold uppercase tracking-wider font-mono">{label}</span>
      </div>
      <span className="font-mono font-bold text-3xl text-abyssal">
